...
#[tauri::command]
pub async fn cancel_download(state: tauri::State<'_, AppState>, id: String) -> Result<(), String> {
    // В Tauri загрузки управляются через браузер, поэтому просто удаляем из списка
